...
const mockCards = [
  { id: 1, name: 'HDFC Regalia', number: '4532****8901', expiry: '08/28', limit: 500000, balance: 78500, dueDate: '2026-03-05', rewards: 12450, gradient: 0, network: 'Visa', spending: [{ cat: 'Shopping', amount: 25000 }, { cat: 'Dining', amount: 12000 }, { cat: 'Travel', amount: 18500 }, { cat: 'Fuel', amount: 8000 }, { cat: 'Other', amount: 15000 }] },
  { id: 2, name: 'SBI SimplyCLICK', number: '5421****3456', expiry: '11/27', limit: 200000, balance: 45200, dueDate: '2026-03-10', rewards: 5680, gradient: 1, network: 'Mastercard', spending: [{ cat: 'Online', amount: 20000 }, { cat: 'Grocery', amount: 8000 }, { cat: 'Entertainment', amount: 7200 }, { cat: 'Utilities', amount: 5000 }, { cat: 'Other', amount: 5000 }] },
  { id: 3, name: 'Axis Magnus', number: '3742****5678', expiry: '05/29', limit: 1000000, balance: 234500, dueDate: '2026-03-15', rewards: 45000, gradient: 2, network: 'Amex', spending: [{ cat: 'Travel', amount: 85000 }, { cat: 'Hotels', amount: 45000 }, { cat: 'Shopping', amount: 54500 }, { cat: 'Dining', amount: 25000 }, { cat: 'Other', amount: 25000 }] },
  { id: 4, name: 'ICICI Amazon Pay', number: '4916****7890', expiry: '02/27', limit: 300000, balance: 23800, dueDate: '2026-03-20', rewards: 3200, gradient: 3, network: 'Visa', spending: [{ cat: 'Amazon', amount: 12000 }, { cat: 'Online', amount: 5800 }, { cat: 'Bills', amount: 3000 }, { cat: 'Other', amount: 3000 }] },
];
...
export default function CreditCardManager() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setCards(res.data?.length ? res.data : mockCards);
      } catch {
        setCards(mockCards);
      } finally {
  ...
  const totalRewards = cards.reduce((s, c) => s + c.rewards, 0);
  ...
  const addCard = () => {
    ...
    setCards([...cards, {
      id: Date.now(), name: newCard.name, number: newCard.number.replace(/\d(?=\d{4})/g, '*'),
      expiry: newCard.expiry || '12/28', limit: +newCard.limit || 100000, balance: 0,
      dueDate: '2026-04-01', rewards: 0, gradient: newCard.gradient, network: 'Visa',
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 overflow-hidden">
              <div className="flex gap-4 transition-transform duration-300" style={{ transform: `translateX(-${selectedCard * 340}px)` }}>
                {cards.map((c, i) => (
                  <div key={c.id} onClick={() => setSelectedCard(i)} className={`min-w-[320px] rounded-2xl p-6 bg-gradient-to-br ${CARD_GRADIENTS[c.gradient % CARD_GRADIENTS.length]} text-white cursor-pointer transition-all duration-300 ${selectedCard === i ? 'scale-105 shadow-2xl ring-2 ring-white/30' : 'opacity-80 hover:opacity-100'}`}>
                    <div className="flex items-start justify-between mb-8">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); toggleShowNumber(c.id); }} className="p-1 rounded-lg hover:bg-white/10">
                        {showNumbers[c.id] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                      </button>
                    </div>
                    <p className="text-xl font-mono tracking-widest mb-6">{showNumbers[c.id] ? c.number.replace(/\*/g, '5') : c.number}</p>
                    <div className="flex justify-between items-end">
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-bold text-lg">₹{c.balance.toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
              ...
              <div className="flex justify-center mb-6">
                <UtilizationRing used={card.balance} limit={card.limit} size={120} />
              </div>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">Credit Limit</span>
                  <span className="font-semibold text-slate-900 dark:text-white">₹{card.limit.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">Outstanding</span>
                  <span className="font-semibold text-red-500">₹{card.balance.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">Available</span>
                  <span className="font-semibold text-emerald-600">₹{(card.limit - card.balance).toLocaleString()}</span>
                </div>
                ...
                <div className="pt-3 border-t border-slate-200 dark:border-slate-700 flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-purple-600">{card.rewards.toLocaleString()} pts</span>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-3">
                {card.spending.map((s, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-slate-600 dark:text-slate-400">{s.cat}: ₹{s.amount.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 animate-fade-in-up" style={{ animationDelay: '500ms' }}>
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                  ...
                  <p className="text-xl font-bold text-slate-900 dark:text-white">₹{card.balance.toLocaleString()}</p>
                </div>
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Minimum Due</p>
                  <p className="text-xl font-bold text-amber-600">₹{Math.round(card.balance * 0.05).toLocaleString()}</p>
                </div>
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50">
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Reward Value</p>
                      <p className="text-lg font-bold text-purple-600">₹{Math.round(card.rewards * 0.25).toLocaleString()}</p>
                    </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 animate-fade-in-up" style={{ animationDelay: '600ms' }}>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={cards.map((c) => ({ name: c.name.split(' ').pop(), used: c.balance, available: c.limit - c.balance }))}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 dashboard-grid animate-fade-in-up" style={{ animationDelay: '700ms' }}>
          {cards.map((c, i) => {
            ...
            return (
              <div key={c.id} className={`bg-white dark:bg-slate-800 rounded-2xl p-4 border ${days <= 3 ? 'border-red-300 dark:border-red-700' : days <= 7 ? 'border-amber-300 dark:border-amber-700' : 'border-slate-200 dark:border-slate-700'}`}>
                ...
                <p className="text-lg font-bold text-slate-900 dark:text-white">₹{c.balance.toLocaleString()}</p>
