  ...
  const getStatusBadge = (bill) => {
    const statuses = {
      pending: { 
        ...
        color: `bg-blue-100 text-blue-700 ${dk ? 'bg-blue-900/30 text-blue-400' : ''}`,
        text: 'Pending'
      },
      awaiting_approval: { 
        icon: <AlertCircle className="w-4 h-4" />, 
        color: `bg-amber-100 text-amber-700 ${dk ? 'bg-amber-900/30 text-amber-400' : ''}`,
        text: 'Needs Approval'
      },
      approved: { 
        icon: <CheckCircle className="w-4 h-4" />, 
        color: `bg-green-100 text-green-700 ${dk ? 'bg-green-900/30 text-green-400' : ''}`,
        text: 'Approved'
      },
      paid: { 
        icon: <Check className="w-4 h-4" />, 
        color: `bg-emerald-100 text-emerald-700 ${dk ? 'bg-emerald-900/30 text-emerald-400' : ''}`,
        text: 'Paid'
      },
      overdue: { 
        icon: <AlertTriangle className="w-4 h-4" />, 
        color: `bg-red-100 text-red-700 ${dk ? 'bg-red-900/30 text-red-400' : ''}`,
        text: 'Overdue'
      },
      rejected: { 
        icon: <XCircle className="w-4 h-4" />, 
        color: `bg-gray-100 text-gray-700 ${dk ? 'bg-slate-700 text-slate-300' : ''}`,
        text: 'Rejected'
      }
    };
  ...
  const headerActions = (
    <div className="flex gap-3">
      <button
        ...
        className={`px-4 py-2 bg-white text-gray-700 border border-gray-300 rounded-lg hover:shadow-md transition-all duration-200 flex items-center gap-2 ${dk ? 'bg-slate-800 text-slate-300 border-slate-600 hover:shadow-slate-900/30' : ''}`}
      >
      ...
      >
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className={`text-gray-600 ${dk ? 'text-slate-400' : ''}`}>Loading bill reminders...</p>
          </div>
        </div>
      </MainLayout>
    );
  ...
  return (
    ...
    >
      ...
      {dashboard && (
        ...
        {/* Filters and Search */}
        <div className={`bg-white rounded-xl shadow-lg p-6 mb-6 ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
          <div className="flex flex-col gap-4">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-all ${
                      filter === f
                        ...
                        : `bg-gray-100 text-gray-700 hover:bg-gray-200 ${dk ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : ''}`
                    }`}
                  >
              ...
              <div className="relative w-full md:w-64">
                <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5 ${dk ? 'text-slate-500' : ''}`} />
                <input
                  type="text"
                  placeholder="Search bills..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className={`w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent ${dk ? 'bg-slate-700 border-slate-600 text-slate-200' : ''}`}
                />
              </div>
            </div>
            ...
            {selectedBills.length > 0 && (
              <div className={`flex items-center justify-between p-4 bg-purple-50 rounded-lg border-2 border-purple-200 ${dk ? 'bg-purple-900/20 border-purple-700' : ''}`}>
                <div className="flex items-center gap-3">
                  <CheckSquare className="w-5 h-5 text-purple-600" />
                  <span className={`font-medium text-gray-700 ${dk ? 'text-slate-300' : ''}`}>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-all ${dk ? 'bg-slate-600 hover:bg-slate-500' : ''}`}
                  >
                    Clear
                  </button>
                </div>
        ...
        {loading ? (
          ...
          <div className={`bg-white rounded-xl shadow-lg p-12 text-center ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
            <Bell className={`w-16 h-16 mx-auto text-gray-300 mb-4 ${dk ? 'text-slate-600' : ''}`} />
            <h3 className={`text-xl font-semibold text-gray-700 mb-2 ${dk ? 'text-slate-300' : ''}`}>No bills found</h3>
            <p className={`text-gray-500 mb-6 ${dk ? 'text-slate-400' : ''}`}>Start by adding your first bill reminder</p>
            <button
              onClick={() => setShowAddModal(true)}
...
const StatCard = ({ title, value, amount, icon, gradient, onClick, pulse }) => {
  ...
  <div
    onClick={onClick}
    className={`bg-white rounded-xl shadow-lg p-6 cursor-pointer transform hover:scale-105 transition-all duration-200 ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''} ${
      pulse ? 'animate-pulse' : ''
    }`}
  >
    <div className="flex items-center justify-between mb-4">
      ...
      <span className={`text-3xl font-bold text-gray-800 ${dk ? 'text-slate-100' : ''}`}>{value}</span>
    </div>
    <h3 className={`text-gray-600 font-medium mb-1 ${dk ? 'text-slate-400' : ''}`}>{title}</h3>
    <p className={`text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>{amount}</p>
  </div>
  );
};
...
const SnoozeModal = ({ bill, onClose, onSnooze }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-2xl shadow-2xl max-w-md w-full ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
        <div className="p-6">
          <div className="flex items-center justify-center mb-4">
            <div className={`w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center ${dk ? 'bg-orange-900/30' : ''}`}>
              <Pause className="w-8 h-8 text-orange-600" />
            </div>
          </div>
          
          <h2 className={`text-2xl font-bold text-gray-800 text-center mb-2 ${dk ? 'text-slate-100' : ''}`}>
            Snooze Bill Reminder
          </h2>
          
          <p className={`text-gray-600 text-center mb-4 ${dk ? 'text-slate-400' : ''}`}>
            {bill.title} - ₹{bill.amount.toLocaleString()}
          </p>

          <div className="mb-6">
            <label className={`block text-sm font-medium text-gray-700 mb-2 ${dk ? 'text-slate-300' : ''}`}>
              Snooze for (days)
            </label>
            <input
              type="number"
              ...
              className={`w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent ${dk ? 'bg-slate-700 border-slate-600 text-slate-200' : ''}`}
              placeholder="Number of days"
            />
            <p className={`mt-2 text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className={`flex-1 px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all font-medium ${dk ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : ''}`}
            >
              Cancel
            </button>
            <button
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-2xl shadow-2xl max-w-md w-full ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
        <div className="p-6">
          <div className="flex items-center justify-center mb-4">
            <div className={`w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center ${dk ? 'bg-blue-900/30' : ''}`}>
          ...
          <p className={`text-gray-600 text-center mb-6 ${dk ? 'text-slate-400' : ''}`}>
          ...
          <div className="space-y-4 mb-6">
            <div>
              ...
              <select
                ...
                className={`w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${dk ? 'bg-slate-700 border-slate-600 text-slate-200' : ''}`}
              >
...
const BillCard = ({ 
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-all duration-200 hover:shadow-xl relative ${dk ? 'bg-slate-800 shadow-slate-900/30 hover:shadow-slate-900/50' : ''}`}>
      {/* Selection Checkbox */}
      <div className="absolute top-4 left-4 z-10">
        <button
          onClick={() => onSelect(bill._id)}
          className={`w-6 h-6 flex items-center justify-center bg-white rounded shadow-md hover:shadow-lg transition-all ${dk ? 'bg-slate-700 shadow-slate-900/30' : ''}`}
        >
          {isSelected ? (
            <CheckSquare className="w-5 h-5 text-purple-600" />
          ) : (
            <Square className={`w-5 h-5 text-gray-400 ${dk ? 'text-slate-500' : ''}`} />
          )}
        </button>
      </div>
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className={`text-xl font-bold text-gray-800 mb-2 ${dk ? 'text-slate-100' : ''}`}>{bill.title}</h3>
        
        <div className="flex items-baseline justify-between mb-4">
          <span className={`text-3xl font-bold text-gray-900 ${dk ? 'text-white' : ''}`}>₹{bill.amount.toLocaleString()}</span>
          {getStatusBadge(bill)}
        </div>

        <div className="space-y-2 mb-4">
          <div className={`flex items-center gap-2 text-sm text-gray-600 ${dk ? 'text-slate-400' : ''}`}>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            onClick={onView}
            className={`px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all ${dk ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : ''}`}
            title="View Details"
          >
          ...
          <button
            onClick={onDelete}
            className={`px-4 py-2 bg-gray-100 text-red-600 rounded-lg hover:bg-red-50 transition-all ${dk ? 'bg-slate-700 hover:bg-red-900/20' : ''}`}
            title="Delete"
          >
            <Trash2 className="w-4 h-4" />
          </button>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-2xl shadow-2xl max-w-md w-full ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
        <div className="p-6">
          <div className="flex items-center justify-center mb-4">
            <div className={`w-16 h-16 bg-red-100 rounded-full flex items-center justify-center ${dk ? 'bg-red-900/30' : ''}`}>
...
const ApprovalModal = ({ bill, onClose, onApprove, onReject }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
        <div className={`p-6 border-b border-gray-200 ${dk ? 'border-slate-700' : ''}`}>
          <div className="flex items-center justify-between">
            <h2 className={`text-2xl font-bold text-gray-800 ${dk ? 'text-slate-100' : ''}`}>Payment Approval</h2>
            <button onClick={onClose} className={`text-gray-400 hover:text-gray-600 ${dk ? 'text-slate-500 hover:text-slate-300' : ''}`}>
              <X className="w-6 h-6" />
            </button>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className={`bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl p-6 mb-6 ${dk ? 'from-purple-900/20 to-blue-900/20' : ''}`}>
            <h3 className={`text-xl font-bold text-gray-800 mb-2 ${dk ? 'text-slate-100' : ''}`}>{bill.title}</h3>
            <div className="text-4xl font-bold text-purple-600 mb-4">
              ₹{bill.amount.toLocaleString()}
            </div>
            <div className={`space-y-2 text-sm text-gray-600 ${dk ? 'text-slate-400' : ''}`}>
              <div className="flex items-center gap-2">
                <Calendar className="w-4 h-4" />
                <span>Due: {new Date(bill.dueDate).toLocaleDateString()}</span>
              </div>
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              rows={3}
              className={`w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent ${dk ? 'bg-slate-700 border-slate-600 text-slate-200' : ''}`}
...
const AddBillModal = ({ bill, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
        <div className={`p-6 border-b border-gray-200 sticky top-0 bg-white z-10 ${dk ? 'border-slate-700 bg-slate-800' : ''}`}>
          <div className="flex items-center justify-between">
            <h2 className={`text-2xl font-bold text-gray-800 ${dk ? 'text-slate-100' : ''}`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 space-y-4">
              <div className={`flex items-center gap-3 p-4 bg-purple-50 rounded-lg ${dk ? 'bg-purple-900/20' : ''}`}>
                <input
                  type="checkbox"
                  id="autoPayEnabled"
                ...
                <label htmlFor="autoPayEnabled" className="flex-1 cursor-pointer">
                  <div className={`font-medium text-gray-800 flex items-center gap-2 ${dk ? 'text-slate-100' : ''}`}>
                    <Zap className="w-5 h-5 text-purple-600" />
                    Enable Auto-Payment
                  </div>
                  <p className={`text-sm text-gray-600 ${dk ? 'text-slate-400' : ''}`}>Automatically process payment when due</p>
                </label>
              </div>

              {formData.autoPayEnabled && (
                <div className={`flex items-center gap-3 p-4 bg-amber-50 rounded-lg ${dk ? 'bg-amber-900/20' : ''}`}>
                  <input
                    type="checkbox"
                    id="requiresApproval"
                  ...
                  <label htmlFor="requiresApproval" className="flex-1 cursor-pointer">
                    ...
                    <p className={`text-sm text-gray-600 ${dk ? 'text-slate-400' : ''}`}>You'll need to approve each payment before it's processed</p>
                  </label>
                </div>
              )}
            </div>
          ...
          <div className={`flex gap-3 pt-6 border-t border-gray-200 ${dk ? 'border-slate-700' : ''}`}>
            <button
              type="button"
              onClick={onClose}
              className={`flex-1 px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all font-medium ${dk ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : ''}`}
...
const BillDetailsModal = ({ bill, onClose, getCategoryIcon, getCategoryColor, getStatusBadge }) => {
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div className={`bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto ${dk ? 'bg-slate-800 shadow-slate-900/30' : ''}`}>
      <div className={`p-6 border-b border-gray-200 ${dk ? 'border-slate-700' : ''}`}>
        <div className="flex items-center justify-between">
          <h2 className={`text-2xl font-bold text-gray-800 ${dk ? 'text-slate-100' : ''}`}>Bill Details</h2>
          <button onClick={onClose} className={`text-gray-400 hover:text-gray-600 ${dk ? 'text-slate-500 hover:text-slate-300' : ''}`}>
            <X className="w-6 h-6" />
          </button>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={`text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>Due Date</label>
            <p className={`text-gray-800 font-medium ${dk ? 'text-slate-100' : ''}`}>{new Date(bill.dueDate).toLocaleDateString()}</p>
          </div>
          <div>
            <label className={`text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>Frequency</label>
            <p className={`text-gray-800 font-medium capitalize ${dk ? 'text-slate-100' : ''}`}>{bill.frequency}</p>
          </div>
          <div>
            <label className={`text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>Reminder Days</label>
            <p className={`text-gray-800 font-medium ${dk ? 'text-slate-100' : ''}`}>{bill.reminderDays} days before</p>
          </div>
          <div>
            <label className={`text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>Auto-Payment</label>
            <p className={`text-gray-800 font-medium ${dk ? 'text-slate-100' : ''}`}>{bill.autoPayEnabled ? '✅ Enabled' : '❌ Disabled'}</p>
          </div>
        </div>
        ...
        {bill.description && (
          <div>
            <label className={`text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>Description</label>
            <p className={`text-gray-800 ${dk ? 'text-slate-100' : ''}`}>{bill.description}</p>
          </div>
        )}
        ...
        {bill.paymentHistory && bill.paymentHistory.length > 0 && (
          <div>
            <h4 className={`font-semibold text-gray-800 mb-3 ${dk ? 'text-slate-100' : ''}`}>Payment History</h4>
            <div className="space-y-2">
              {bill.paymentHistory.map((payment, index) => (
                <div key={index} className={`flex items-center justify-between p-3 bg-gray-50 rounded-lg ${dk ? 'bg-slate-900' : ''}`}>
                  <div>
                    <p className={`font-medium text-gray-800 ${dk ? 'text-slate-100' : ''}`}>₹{payment.amount.toLocaleString()}</p>
                    <p className={`text-sm text-gray-500 ${dk ? 'text-slate-400' : ''}`}>{new Date(payment.paidDate).toLocaleDateString()}</p>
                  </div>
                  <span className={`text-sm text-gray-600 capitalize ${dk ? 'text-slate-400' : ''}`}>{payment.paymentMethod}</span>
                </div>
              ))}
            </div>
          </div>
