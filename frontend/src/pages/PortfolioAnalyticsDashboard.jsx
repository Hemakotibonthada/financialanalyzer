    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-700">No Portfolio Data</h2>
          <p className="text-gray-500 mt-2">Add investments to start tracking your portfolio</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Advanced investment analysis and performance metrics</p>
          </div>
        ...
        {activeTab === 'performance' && (
          <div className="space-y-6">
            {/* Returns by Period */}
            <div className="bg-white rounded-lg shadow-md p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Returns by Period</h3>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {portfolioData.returnsByPeriod && Object.entries(portfolioData.returnsByPeriod).map(([period, value]) => (
                  <div key={period} className="p-4 bg-gray-50 rounded-lg text-center">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Monthly Returns</h3>
              <div className="h-80">
                  ...
                  {portfolioData.topPerformers?.map((holding, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                      <div>
                        ...
                        <div className="text-sm text-gray-600">{holding.type}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-lg font-bold text-green-600">+{holding.return?.toFixed(2)}%</div>
                        <div className="text-sm text-gray-600">₹{holding.gain?.toLocaleString()}</div>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {portfolioData.underperformers?.map((holding, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-sm text-gray-600">₹{holding.loss?.toLocaleString()}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'risk' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Risk vs Return Analysis</h3>
              <div className="h-80">
            ...
            {/* Concentration Risk */}
            <div className="bg-white rounded-lg shadow-md p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Concentration Risk</h3>
              <div className="space-y-4">
                <div className="p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                  <div className="flex items-start space-x-3">
                    <AlertTriangle className="w-5 h-5 text-yellow-600 mt-0.5 flex-shrink-0" />
                    <div>
                      <div className="font-semibold text-gray-900">Top 5 Holdings</div>
        ...
        {activeTab === 'allocation' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Target vs Actual Allocation</h3>
              <div className="space-y-4">
            ...
            {portfolioData.rebalanceRecommendations && (
              <div className="bg-white rounded-lg shadow-md p-6">
                <h3 className="text-xl font-bold text-gray-900 mb-4">Rebalancing Recommendations</h3>
                <div className="space-y-3">
                  {portfolioData.rebalanceRecommendations.map((rec, index) => (
                    <div key={index} className={`p-4 border-l-4 rounded ${
                      rec.action === 'buy' ? 'bg-green-50 border-green-500' : 'bg-red-50 border-red-500'
                    }`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="font-semibold text-gray-900">{rec.holding}</div>
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-sm text-gray-900">₹{rec.amount?.toLocaleString()}</div>
                        </div>
                      </div>
                    </div>
                  ))}
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600">{holding.type} • {holding.quantity} units</p>
        </div>
        <div className="text-right">
          <div className="text-2xl font-bold text-gray-900">₹{holding.currentValue?.toLocaleString()}</div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="p-3 bg-gray-50 rounded-lg">
          ...
          <div className="text-sm font-bold text-gray-900">₹{holding.invested?.toLocaleString()}</div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-600">Avg Price</div>
          <div className="text-sm font-bold text-gray-900">₹{holding.avgPrice?.toLocaleString()}</div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-600">Current Price</div>
          <div className="text-sm font-bold text-gray-900">₹{holding.currentPrice?.toLocaleString()}</div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
