    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${dk ? 'bg-slate-900' : 'bg-slate-50'}`}>
        <div className="text-center">
          ...
          <p className={`${dk ? 'text-slate-400' : 'text-slate-600'}`}>Loading security data...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${dk ? 'bg-slate-900' : 'bg-slate-50'}`}>
        <div className="text-center">
          ...
          <p className={`${dk ? 'text-slate-400' : 'text-slate-600'} mb-4`}>{error}</p>
    ...
    <div className={`min-h-screen ${dk ? 'bg-slate-900' : 'bg-slate-50'} p-4 md:p-8`}>
      <div className="max-w-6xl mx-auto space-y-6 animate-fade-in-up">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className={`${dk ? 'text-slate-400' : 'text-slate-500'} mt-1`}>Monitor and manage your account security</p>
          </div>
          <div className="flex items-center gap-3">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl ${securityScore >= 80 ? `${dk ? 'bg-green-900/30' : 'bg-green-100'}` : `${dk ? 'bg-yellow-900/30' : 'bg-yellow-100'}`}`}>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 dashboard-grid">
              ...
              <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
                <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-900'} mb-4`}>Security Score</h3>
              ...
              {/* Radar */}
              <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
                <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-900'} mb-4`}>Security Breakdown</h3>
            ...
            <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {twoFaSteps.map(step => (
                  <div key={step.id} className={`p-4 rounded-xl border-2 text-center ${step.done ? `border-green-500 ${dk ? `bg-green-900/20` : `bg-green-50`}` : `${dk ? 'border-slate-700' : 'border-slate-200'} ${dk ? `bg-slate-700/30` : `bg-slate-50`}`}`}>
                    ...
                    <p className={`text-sm font-medium ${step.done ? `${dk ? 'text-green-400' : 'text-green-700'}` : `${dk ? 'text-slate-400' : 'text-slate-500'}`}`}>{step.label}</p>
            ...
            <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                {[['Min 12 chars', true], ['Uppercase', true], ['Numbers', true], ['Special chars', false]].map(([label, ok]) => (
                  <div key={label} className={`flex items-center gap-2 ${dk ? 'text-slate-300' : 'text-slate-600'}`}>
            ...
            <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={item.label} className={`p-4 rounded-xl ${dk ? 'bg-green-900/20' : 'bg-green-50'} border ${dk ? 'border-green-800' : 'border-green-200'}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <ShieldCheck className="w-4 h-4 text-green-500" />
                      <span className={`text-sm font-medium ${dk ? 'text-green-300' : 'text-green-800'}`}>{item.label}</span>
                    </div>
                    <p className={`text-xs ${dk ? 'text-green-400' : 'text-green-600'}`}>{item.status}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
              ...
              <div className="space-y-3">
                {securityEvents.length === 0 ? (
                  <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'} text-center py-4`}>No recent security events.</p>
                ) : securityEvents.map(evt => (
                  <div key={evt.id} className={`flex items-center gap-3 p-3 rounded-xl ${dk ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                    ...
                    <div className="flex-1">
                      <p className={`text-sm ${dk ? 'text-white' : 'text-slate-900'}`}>{evt.event}</p>
                      <p className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'}`}>{evt.time}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'sessions' && (
          <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
            <div className="flex items-center justify-between mb-4">
              <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-900'}`}>Active Sessions</h3>
            ...
            <div className="space-y-3">
              {sessions.length === 0 ? (
                <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'} text-center py-8`}>No active sessions found.</p>
              ) : sessions.map(session => (
                <div key={session.id} className={`flex items-center justify-between p-4 rounded-xl ${session.current ? `${dk ? `bg-blue-900/20` : `bg-blue-50`} border ${dk ? 'border-blue-800' : 'border-blue-200'}` : `${dk ? `bg-slate-700/50` : `bg-slate-50`}`}`}>
                  <div className="flex items-center gap-3">
                    <Laptop className="w-6 h-6 text-slate-500" />
                    <div>
                      <p className={`font-medium text-sm ${dk ? `text-white` : `text-slate-900`}`}>{session.device} {session.current && <span className={`text-xs ${dk ? 'bg-blue-900/40' : 'bg-blue-100'} ${dk ? `text-blue-400` : `text-blue-700`} px-2 py-0.5 rounded-full ml-2`}>Current</span>}</p>
                      <p className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'}`}>{session.location} • {session.lastActive}</p>
                    </div>
                  </div>
                  {!session.current && (
        ...
        {activeTab === 'history' && (
          <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
            <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-900'} mb-4`}>Login History</h3>
            <div className="space-y-3">
              {loginHistory.length === 0 ? (
                <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'} text-center py-8`}>No login history available.</p>
              ) : loginHistory.map(login => {
                const DeviceIcon = getDeviceIcon(login.icon);
                return (
                  <div key={login.id} className={`flex items-center gap-4 p-4 rounded-xl ${dk ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                    <DeviceIcon className="w-6 h-6 text-slate-500 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium ${dk ? 'text-white' : 'text-slate-900'}`}>{login.device}</p>
                      <div className={`flex items-center gap-3 text-xs ${dk ? 'text-slate-400' : 'text-slate-500'}`}>
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full ${login.status === `success` ? `${dk ? `bg-green-900/40` : `bg-green-100`} ${dk ? `text-green-400` : `text-green-700`}` : `${dk ? `bg-red-900/40` : `bg-red-100`} ${dk ? `text-red-400` : 'text-red-700'}`}`}>{login.status}</span>
                  </div>
                );
              })}
            </div>
        ...
        {activeTab === 'privacy' && (
          <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
            <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-900'} mb-4`}>Privacy Settings</h3>
            <div className="space-y-4">
              {[
                ...
                <div key={item.key} className={`flex items-center justify-between p-4 rounded-xl ${dk ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                  <div>
                    <p className={`text-sm font-medium ${dk ? 'text-white' : 'text-slate-900'}`}>{item.label}</p>
                    <p className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'}`}>{item.desc}</p>
                  </div>
                  <button onClick={() => setPrivacy(p => ({ ...p, [item.key]: !p[item.key] }))}
                    className={`w-11 h-6 rounded-full transition-colors relative ${privacy[item.key] ? 'bg-blue-600' : `${dk ? 'bg-slate-600' : 'bg-slate-300'}`}`}>
        ...
        {activeTab === 'danger' && (
          <div className="space-y-4">
            <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
              <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-900'} mb-4`}>Export Your Data</h3>
              <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'} mb-4`}>Download a copy of all your financial data in JSON format.</p>
            ...
            <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border-2 ${dk ? 'border-red-800' : 'border-red-200'}`}>
              <h3 className={`text-lg font-semibold ${dk ? 'text-red-400' : 'text-red-600'} mb-2`}>Delete Account</h3>
              <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'} mb-4`}>Permanently delete your account and all associated data. This action cannot be undone.</p>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 w-full max-w-md border ${dk ? 'border-red-800' : 'border-red-200'}`}>
            <div className="text-center mb-6">
              <AlertTriangle className="w-12 h-12 text-red-500 mx-auto mb-3" />
              <h3 className={`text-lg font-bold ${dk ? 'text-white' : 'text-slate-900'}`}>Are you sure?</h3>
              <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'} mt-2`}>This will permanently delete your account and all data. This cannot be undone.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} className={`flex-1 px-4 py-2 rounded-xl text-sm font-medium ${dk ? `bg-slate-700` : `bg-slate-100`} ${dk ? 'text-slate-400' : 'text-slate-600'} ${dk ? `hover:bg-slate-600` : `hover:bg-slate-200`}`}>Cancel</button>
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 w-full max-w-md border ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
            <div className="flex items-center justify-between mb-4">
              <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-900'}`}>Export Data</h3>
              <button onClick={() => setShowExportModal(false)} className={`p-1 ${dk ? 'hover:bg-slate-700' : 'hover:bg-slate-100'} rounded-lg`}><X className="w-5 h-5 text-slate-400" /></button>
            </div>
            <div className="space-y-3">
              {['Transactions', 'Budgets', 'Goals', 'Settings', 'Login History'].map(item => (
                <label key={item} className={`flex items-center gap-3 p-3 rounded-xl ${dk ? 'bg-slate-700/50' : 'bg-slate-50'} cursor-pointer`}>
                  <input type="checkbox" defaultChecked className="rounded text-blue-600" />
                  <span className={`text-sm ${dk ? 'text-white' : 'text-slate-900'}`}>{item}</span>
                </label>
              ))}
            </div>
            <div className="flex gap-3 justify-end mt-6">
              <button onClick={() => setShowExportModal(false)} className={`px-4 py-2 rounded-xl text-sm font-medium ${dk ? 'text-slate-400' : 'text-slate-600'}`}>Cancel</button>
