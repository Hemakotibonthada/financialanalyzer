...
export default function FinancialScorecard() {
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${dk ? 'bg-slate-900' : 'bg-slate-50'}`}>
        <div className="text-center">
          ...
          <p className={`${dk ? 'text-slate-400' : 'text-slate-600'}`}>Loading financial scorecard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <MainLayout title="Financial Scorecard">
    <div className={`min-h-screen ${dk ? 'bg-slate-900' : 'bg-slate-50'} p-4 md:p-6 space-y-6`}>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'} mt-1`}>Your comprehensive financial health assessment</p>
        </div>
        <div className="flex gap-2">
          <button className={`${dk ? 'bg-slate-700' : 'bg-slate-100'} ${dk ? 'text-slate-300' : 'text-slate-700'} rounded-xl text-sm px-4 py-2 flex items-center gap-2`}>
      ...
      {/* Rotating Tip */}
      <div className={`bg-gradient-to-r ${dk ? `from-blue-900/20` : `from-blue-50`} ${dk ? 'to-cyan-900/20' : 'to-cyan-50'} rounded-2xl p-4 border ${dk ? `border-blue-800` : `border-blue-100`} flex items-center gap-3`}>
        <Zap className="w-5 h-5 text-blue-600 flex-shrink-0" />
        <p className={`text-sm ${dk ? 'text-blue-300' : 'text-blue-700'} transition-all`}>{tips[tipIndex]}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm flex flex-col items-center`}>
          <h2 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'} mb-4`}>Overall Financial Score</h2>
          <svg width="220" height="190" viewBox="0 0 220 190">
            ...
            <text x="110" y="105" textAnchor="middle" className={`${dk ? 'fill-white' : 'fill-slate-800'}`} fontSize="42" fontWeight="bold">{animatedScore}</text>
          ...
          <p className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'} mt-1`}>Potential: {overallScore + totalImpact} with all improvements</p>
        </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
          {categoryScoresWithIcons.map((c, i) => {
            ...
            <button key={i} onClick={() => setSelectedCategory(selectedCategory === i ? null : i)}
              className={`${dk ? `bg-slate-800` : `bg-white`} rounded-2xl p-4 border shadow-sm text-left transition-all ${selectedCategory === i ? `${dk ? `border-blue-400` : `border-blue-500`} ring-2 ${dk ? `ring-blue-900` : `ring-blue-200`}` : `${dk ? `border-slate-700` : `border-slate-200`} ${dk ? `hover:border-blue-600` : `hover:border-blue-300`}`}`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-sm font-medium ${dk ? 'text-white' : 'text-slate-800'}`}>{c.category}</span>
              </div>
              <div className="flex items-end justify-between">
                <p className={`text-2xl font-bold ${dk ? 'text-white' : 'text-slate-800'}`}>{c.score}</p>
              ...
              <div className={`mt-2 w-full ${dk ? 'bg-slate-700' : 'bg-slate-100'} rounded-full h-2`}>
                <div className="h-2 rounded-full transition-all" style={{ width: `${c.score}%`, backgroundColor: c.color }} />
              </div>
              {selectedCategory === i && (
                <div className={`mt-3 pt-3 border-t ${dk ? 'border-slate-700' : 'border-slate-100'} space-y-1`}>
                  {(c.tips || []).map((tip, j) => (
                    <p key={j} className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'} flex items-start gap-1`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm`}>
          <h2 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'} mb-4`}>Score Trend</h2>
        ...
        <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm`}>
          <h2 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'} mb-4`}>Score Breakdown vs Peers</h2>
          <ResponsiveContainer width="100%" height={260}>
      ...
      <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm`}>
        <h2 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'} mb-4`}>Peer Comparison</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className={`border-b ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
                <th className={`text-left py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium`}>Metric</th>
                <th className={`text-right py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium`}>Your Score</th>
                <th className={`text-right py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium`}>Peer Avg</th>
                <th className={`text-right py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium`}>Percentile</th>
                <th className={`py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium text-center w-32`}>Position</th>
              </tr>
            </thead>
            <tbody>
              {peerComparison.map((p, i) => (
                <tr key={i} className={`border-b ${dk ? 'border-slate-700/50' : 'border-slate-100'}`}>
                  <td className={`py-3 px-3 ${dk ? 'text-white' : 'text-slate-800'} font-medium`}>{p.metric}</td>
                  <td className="py-3 px-3 text-right font-bold text-blue-600">{p.you}%</td>
                  <td className={`py-3 px-3 text-right ${dk ? 'text-slate-400' : 'text-slate-500'}`}>{p.peerAvg}%</td>
                  ...
                  <td className="py-3 px-3">
                    <div className={`w-full ${dk ? 'bg-slate-700' : 'bg-slate-100'} rounded-full h-2 relative`}>
      ...
      <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm`}>
        ...
        <div className="space-y-3">
          {recommendations.map((r, i) => (
            <div key={i} className={`rounded-xl border ${dk ? 'border-slate-700' : 'border-slate-100'} ${dk ? 'bg-slate-700/30' : 'bg-slate-50'} overflow-hidden`}>
              <button onClick={() => setExpandedRec(expandedRec === i ? null : i)}
                className={`w-full flex items-center gap-4 p-4 text-left ${dk ? 'hover:bg-slate-700/50' : 'hover:bg-slate-100'} transition-colors`}>
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${r.priority === `High` ? `${dk ? 'bg-red-900/30` : `bg-red-100`}` : r.priority === `Medium` ? `${dk ? `bg-amber-900/30' : 'bg-amber-100'}` : `${dk ? `bg-blue-900/30` : `bg-blue-100`}`}`}>
                ...
                <div className="flex-1">
                  <p className={`text-sm font-medium ${dk ? 'text-white' : 'text-slate-800'}`}>{r.title}</p>
                  <p className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'}`}>{r.category} • {r.priority} Priority</p>
                </div>
              ...
              {expandedRec === i && (
                <div className={`px-4 pb-4 pt-0 border-t ${dk ? 'border-slate-700' : 'border-slate-100'}`}>
                  <p className={`text-sm ${dk ? 'text-slate-300' : 'text-slate-600'} mt-3`}>{r.desc}</p>
      ...
      <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm`}>
        <h2 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'} mb-4 flex items-center gap-2`}>
          ...
          <span className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'} font-normal ml-2`}>{earnedBadges}/{badges.length} earned</span>
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {badges.map((b, i) => (
            <div key={i} className={`p-4 rounded-xl border text-center transition-all ${b.earned ? `${dk ? `bg-amber-900/10` : `bg-amber-50`} ${dk ? `border-amber-800` : `border-amber-200`}` : `${dk ? `bg-slate-700/30` : `bg-slate-50`} ${dk ? `border-slate-700` : `border-slate-100`} opacity-75`}`}>
              <div className="text-3xl mb-2">{b.icon}</div>
              <p className={`text-sm font-medium ${dk ? 'text-white' : 'text-slate-800'}`}>{b.name}</p>
              <p className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'} mt-1`}>{b.desc}</p>
              {b.earned ? (
                <p className={`text-xs ${dk ? 'text-amber-400' : 'text-amber-600'} mt-2 flex items-center justify-center gap-1`}>
                  <Star className="w-3 h-3" /> {b.date}
                </p>
              ) : (
                <div className="mt-2">
                  <div className={`w-full ${dk ? 'bg-slate-600' : 'bg-slate-200'} rounded-full h-1.5`}>
                    <div className="h-1.5 rounded-full bg-blue-500" style={{ width: `${b.progress}%` }} />
                  </div>
                  <p className={`text-xs ${dk ? 'text-slate-400' : 'text-slate-500'} mt-1`}>{b.progress}%</p>
                </div>
              )}
            </div>
          ))}
      ...
      <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className={`border-b ${dk ? 'border-slate-700' : 'border-slate-200'}`}>
                <th className={`text-left py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium`}>Month</th>
                <th className={`text-center py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium`}>Overall</th>
                {categoryScores.map(c => (
                  <th key={c.category} className={`text-center py-3 px-3 ${dk ? 'text-slate-400' : 'text-slate-500'} font-medium`}>{c.category}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {monthlyHistory.map((row, i) => (
                <tr key={i} className={`border-b ${dk ? 'border-slate-700/50' : 'border-slate-100'}`}>
                  <td className={`py-3 px-3 ${dk ? 'text-white' : 'text-slate-800'} font-medium`}>{row.month}</td>
                  ...
                  <td className={`py-3 px-3 text-center ${dk ? 'text-slate-300' : 'text-slate-600'}`}>{row.savings}</td>
                  <td className={`py-3 px-3 text-center ${dk ? 'text-slate-300' : 'text-slate-600'}`}>{row.investment}</td>
                  <td className={`py-3 px-3 text-center ${dk ? 'text-slate-300' : 'text-slate-600'}`}>{row.debt}</td>
                  <td className={`py-3 px-3 text-center ${dk ? 'text-slate-300' : 'text-slate-600'}`}>{row.insurance}</td>
                  <td className={`py-3 px-3 text-center ${dk ? 'text-slate-300' : 'text-slate-600'}`}>{row.tax}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Score Impact Chart */}
      <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-sm`}>
        <h2 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'} mb-4`}>Potential Score Impact</h2>
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className={`${dk ? 'bg-slate-800' : 'bg-white'} rounded-2xl p-6 w-full max-w-md border ${dk ? 'border-slate-700' : 'border-slate-200'} shadow-xl`}>
            <div className="flex items-center justify-between mb-6">
              <h3 className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'}`}>Share Scorecard</h3>
              <button onClick={() => setShowShareModal(false)} className={`p-1 rounded-lg ${dk ? 'hover:bg-slate-700' : 'hover:bg-slate-100'}`}>
            ...
            <div className="text-center mb-6">
              ...
              <p className={`text-lg font-semibold ${dk ? 'text-white' : 'text-slate-800'} mt-3`}>Financial Score: {getGrade(overallScore).grade}</p>
              <p className={`text-sm ${dk ? 'text-slate-400' : 'text-slate-500'}`}>February 2026</p>
            </div>
            <div className="space-y-2 mb-6">
              {categoryScores.map((c, i) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  <span className={`${dk ? 'text-slate-300' : 'text-slate-600'}`}>{c.category}</span>
                  <span className={`font-medium ${dk ? 'text-white' : 'text-slate-800'}`}>{c.score}/100</span>
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowShareModal(false)} className={`flex-1 ${dk ? 'bg-slate-700' : 'bg-slate-100'} ${dk ? 'text-slate-300' : 'text-slate-700'} rounded-xl text-sm py-2`}>Cancel</button>
