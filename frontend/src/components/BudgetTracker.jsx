    ...
    return (
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 mb-4">{budgetData?.message || 'No budget configured'}</p>
          {budgetData?.recommendation && (
            <p className="text-sm text-gray-400">{budgetData.recommendation}</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-lg shadow">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">Budget Tracker</h3>
        <p className="text-sm text-gray-600 mt-1">Monthly budget progress</p>
      </div>
            ...
            <div className="space-y-2">
              {budgetData.alerts.map((alert, index) => (
                <div key={index} className={`p-3 rounded-lg ${
                  alert.severity === 'over' ? 'bg-red-50 border border-red-200' : 'bg-orange-50 border border-orange-200'
                }`}>
                  <div className="flex items-center">
                    ...
                    <span className={`text-sm font-medium ${
                      alert.severity === 'over' ? 'text-red-800' : 'text-orange-800'
                    }`}>
                      {alert.category}
                    </span>
                  </div>
                  <p className={`text-xs mt-1 ${
                    alert.severity === 'over' ? 'text-red-600' : 'text-orange-600'
                  }`}>
                    {alert.message}
                  </p>
                </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-gray-900">Projected Month End</span>
              </div>
              <span className={`text-sm font-medium ${
